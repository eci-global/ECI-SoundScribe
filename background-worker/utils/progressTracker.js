...
export class ProgressTracker {
  ...
  async complete(results = {}) {
    ...
    console.log(`📊 Final results summary:`, {
      ...
      hasCoaching: !!results.coaching,
      processingTime: results.processingTime || 'unknown'
    });

    try {
      const updateResult = await updateRecordingStatus(
        this.recordingId, 
        'completed', 
        100,
        null
      ...
      if (!updateResult.success) {
        console.error(`❌ Failed to update status to completed:`, updateResult.error);
        console.log(`🔄 Attempting fallback to transcribed status...`);
        
        // Fallback to transcribed status
        const fallbackResult = await updateRecordingStatus(
          this.recordingId, 
          'transcribed', 
          100,
          `Status update fallback: ${updateResult.error}`
        );
        
        if (!fallbackResult.success) {
          console.error(`❌ Fallback status update also failed:`, fallbackResult.error);
          throw new Error(`Critical: Cannot update recording status - ${fallbackResult.error}`);
        } else {
          console.log(`✅ Fallback successful - recording marked as transcribed`);
        }
      } else {
      ...
      console.error(`❌ Critical error in completion process:`, error);
      // Still try to log the attempt
      await logProcessingActivity(this.recordingId, 'processing_completion_error', {
        error: error.message,
        finalResults: results
      });
      throw error;
