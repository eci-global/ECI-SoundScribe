...
Write-Host "
📋 Step 2: Updating server code..." -ForegroundColor Yellow
...
if (Test-Path $serverPath) {
    ...
    $content = $content -replace 
        'whisperEndpoint = process\.env\.AZURE_OPENAI_WHISPER_ENDPOINT \|\| this\.endpoint',
        'transcribeEndpoint = process.env.AZURE_OPENAI_TRANSCRIBE_ENDPOINT || this.endpoint'
    
    $content = $content -replace 
        'whisperApiKey = process\.env\.AZURE_OPENAI_WHISPER_API_KEY \|\| this\.apiKey',
        'transcribeApiKey = process.env.AZURE_OPENAI_TRANSCRIBE_API_KEY || this.apiKey'
    
    # Replace API calls from /deployments/whisper/ to /deployments/gpt-4o-transcribe/
    $content = $content -replace 
        '/deployments/\$\{[^}]*whisper[^}]*\}/audio/transcriptions',
        '/deployments/${this.transcribeDeployment}/audio/transcriptions'
...
Write-Host "
🚀 Step 3: Deploying updated code..." -ForegroundColor Yellow
...
try {
    ...
    Compress-Archive -Path @(
        ...
        "utils",
        "web.config"
    ) -DestinationPath $deploymentZip -Force
...
Write-Host "
🔄 Step 4: Restarting app service..." -ForegroundColor Yellow
...
Write-Host "
🎉 Migration completed!" -ForegroundColor Green
...
Write-Host "   Run: .\upgrade-azure-gpt4o-transcribe.ps1" -ForegroundColor Yellow
