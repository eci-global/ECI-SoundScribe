...
export class FileProcessor {
  ...
  async processRecording(recordingId, processingOptions = {}) {
    ...
    try {
      ...
      let originalBuffer = downloadResult.buffer; // Store original buffer for retry attempts
      ...
      if (!transcriptionResult.success) {
        ...
        // Extract error message from the correct nested structure
        let errorMessage = 'Transcription failed';
        if (transcriptionResult.error?.error?.message) {
          // Handle nested error structure: { error: { message: '...' } }
          errorMessage += `: ${transcriptionResult.error.error.message}`;
        } else if (transcriptionResult.error?.message) {
        ...
        if (errorMessage.includes('corrupted') || errorMessage.includes('unsupported')) {
          ...
          try {
            // Attempt with original buffer first (sometimes it's a transient issue)
            console.log('🔄 Attempting retry with original file buffer...');
            ...
            if (originalTranscriptionResult.success) {
              ...
              // If original buffer still fails, try converting to MP3 as a last resort
              console.warn('🔄 Original buffer retry failed, attempting to convert to MP3 and retry...');
              const mp3Buffer = await this.audioProcessor.compressToMp3(originalBuffer, filename);
              if (!mp3Buffer) {
                throw new Error('Failed to convert to MP3 for retry');
              }
              ...
              if (mp3TranscriptionResult.success) {
                ...
                throw new Error(`Automatic retry failed with both original and MP3 formats: ${mp3TranscriptionResult.error?.error?.message || mp3TranscriptionResult.error?.message || 'Unknown error'}`);
              }
            }
          } catch (retryError) {
            console.error('❌ Automatic retry mechanism failed:', retryError);
            throw new Error(`Transcription failed after automatic retry attempts: ${retryError.message}`);
          }
        } else {
          // If not a corrupted file error, re-throw the original error
