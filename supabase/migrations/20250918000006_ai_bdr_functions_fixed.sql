-- AI BDR Dynamic Loading Functions - Fixed Version
-- Created: 2025-09-18
-- Purpose: Replace problematic migration 20250918000004 with constraint-safe version

-- =========================================
-- ENSURE UNIQUE CONSTRAINT EXISTS FIRST
-- =========================================

-- Create the unique constraint if it doesn't exist
DO $$
BEGIN
    -- Drop and recreate constraint to ensure it exists properly
    DROP INDEX IF EXISTS ai_prompt_templates_name_category_unique CASCADE;

    -- Recreate with explicit naming
    CREATE UNIQUE INDEX ai_prompt_templates_name_category_unique
    ON ai_prompt_templates(name, category);

    RAISE NOTICE 'Unique constraint created/recreated successfully';
EXCEPTION
    WHEN OTHERS THEN
...
-- =========================================
-- BDR PROMPT TEMPLATE SEED DATA - SAFE INSERT
-- =========================================

-- Test the constraint before proceeding
DO $$
BEGIN
    -- Test that constraint works
    INSERT INTO ai_prompt_templates (name, category, description, template, variables, is_active)
    VALUES ('test_constraint', 'custom', 'Test', 'Test template', '[]'::jsonb, false)
    ON CONFLICT (name, category) DO NOTHING;

    -- Clean up test
    DELETE FROM ai_prompt_templates WHERE name = 'test_constraint' AND category = 'custom';

    RAISE NOTICE 'Constraint validation successful - proceeding with seed data';
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Constraint validation failed: %. Cannot proceed with ON CONFLICT operations.', SQLERRM;
END $$;

-- Insert default BDR prompt templates using safe ON CONFLICT
INSERT INTO ai_prompt_templates (name, category, description, template, variables, is_active)
VALUES
...
INSERT INTO ai_configuration_history (
  ...
  'bdr_dynamic_loading_fixed',
  gen_random_uuid(),
  'INSERT',
  NULL, -- System operation (no specific user)
  NULL,
  jsonb_build_object(
    'description', 'BDR dynamic loading functions implemented for AI Control Center (fixed version)',
